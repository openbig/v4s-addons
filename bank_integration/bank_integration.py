...
#        'payment_delay': fields.integer('Delay of Direct Payment', help= "How many days add to payment date when Preferred Date is set to Directly. \
#                                Usualy bank systems accept dates for next day."),
...
#        return self.pool.get('res.company').search(cursor, uid, [('parent_id', '=', False)])[0]

    _defaults = {
        'company_id': lambda self,cr,uid,c: self.pool.get('res.users').browse(cr, uid, uid, c).company_id.id,
        'partner_matching': 'by_bank_only',
        'invoice_matching': 'full_matching',
...
#    _description = __doc__
    ...
    _columns = {
        ...
        'statement_ids': fields.one2many('account.bank.statement', 'banking_id', 'Statements', readonly=False,),
    }
    _defaults = {
...
#    _description = __doc__
    ...
    _columns = {
        ...
        'pay_order_ids': fields.one2many('payment.order', 'banking_id', 'Payment Orders', readonly=False, ),
    }
    _defaults = {
        'date': lambda *a: time.strftime('%Y-%m-%d %H:%M:%S'),
        'user_id': lambda self, cursor, uid, context: uid,
